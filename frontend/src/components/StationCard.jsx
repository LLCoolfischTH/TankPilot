    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1, marginRight: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 6 }}>
            ...
            {isBest && <span style={{ background: '#2563eb', color: '#fff', borderRadius: 4, padding: '2px 7px', fontSize: 11 }}>Beste Wahl</span>}
            {!station.isOpen && <span style={{ background: '#fef2f2', color: '#dc2626', borderRadius: 4, padding: '2px 7px', fontSize: 11, border: '1px solid #fca5a5' }}>Geschlossen</span>}
            <span style={{ background: srcBg, color: srcClr, borderRadius: 4, padding: '2px 7px', fontSize: 11, border: `1px solid ${srcClr}40` }}>{srcLbl}</span>
          </div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 10, fontSize: 13, flexWrap: 'wrap', alignItems: 'center' }}>
        <span>
          ...
          {hasLocal && <span style={{ color: '#888', fontSize: 11, marginLeft: 5 }}>({station.priceLocal.toFixed(2)} {station.currency})</span>}
        </span>
        <span>
      ...
      {open && (
        <div style={{ marginTop: 8, background: '#f8faff', borderRadius: 8, padding: 12, fontSize: 12, lineHeight: 1.9, border: '1px solid #dbeafe' }}>
          ...
          <div><span style={{ color: '#555' }}>① Tankkosten</span><span style={{ float: 'right' }}>{breakdown.fuelCost.toFixed(2)} €</span></div>
          <div style={{ color: '#888', marginLeft: 16, marginBottom: 4, fontFamily: 'monospace', fontSize: 11 }}>{station.pricePerLiter.toFixed(3)} €/L × Tankmenge{hasLocal && ` (${station.priceLocal.toFixed(2)} ${station.currency} → EUR)`}</div>
          <div><span style={{ color: '#555' }}>② Umwegkosten (Hin + Rück)</span><span style={{ float: 'right' }}>{breakdown.detourCost.toFixed(2)} €</span></div>
          <div style={{ color: '#888', marginLeft: 16, marginBottom: 8, fontFamily: 'monospace', fontSize: 11 }}>{station.distanceKm} km × 2 = {(station.distanceKm * 2).toFixed(1)} km → {breakdown.detourFuelLiters.toFixed(2)} L</div>
          <div style={{ borderTop: '1px solid #bfdbfe', paddingTop: 6, fontWeight: 700 }}>
            <span>① + ② Gesamtkosten</span><span style={{ float: 'right' }}>{breakdown.totalCost.toFixed(2)} €</span>
          </div>
          ...
          <div style={{ marginTop: 6, color: srcClr, background: srcBg, borderRadius: 4, padding: '4px 8px', fontSize: 11 }}>Quelle: {srcLbl}</div>
        </div>
      )}
    </div>
  );
